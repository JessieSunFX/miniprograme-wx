...
Page({
  ...
  gotoExchange (){
    wx.navigateTo({
      url: '../diamondExchange/diamondExchange'
    })
  },
    ...
    app.myReq({
      ...
      if (res.statusCode === 200) {
        if (res.data.retCode === "001") {//token过期
          ...
          //判断是否有数据，有则取数据
          if (res.data.diamondList.length != 0) {
            for (let i = 0; i < res.data.diamondList.length;i++){
              res.data.diamondList[i].date = res.data.diamondList[i].time.substr(0,10);
              res.data.diamondList[i].time = res.data.diamondList[i].time.substr(11);
            }
            let diamondList = [];
            //如果isFromSearch是true从data中取出数据，否则先从原来的数据继续添加
            that.data.isFromSearch ? diamondList = res.data.diamondList : diamondList = that.data.diamondList.concat(res.data.diamondList);  //TODO ...............
            that.setData({
              diamondBalance: res.data.diamondBalance,
              diamondIncome: res.data.diamondIncome,
              diamondExpenses: res.data.diamondExpenses,
              diamondList: diamondList, //获取数据数组
